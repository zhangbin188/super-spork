...
function isVideoHost(host) {
    ...
    return h.includes('googlevideo.com') || h.includes('gvt1.com') ||
        h.includes('youtube.com') || h.includes('ytimg.com') ||
        h.includes('tiktokcdn.com') || h.includes('muscdn.com') ||
...
function isMediaHost(host) {
    return isVideoHost(host) || host.includes('telegram');
}
...
async function handleTCPOutBoundOptimized(remote, address, port, initData, ws, proxyHeader, opts = {}) {
    if (remote._active) return;
    remote._active = true;

    const media = !!opts.media;
    const raceDelay = (RACE_ENABLED && proxyConfig.host) ? (media ? MEDIA_RACE_DELAY_MS : GEN_RACE_DELAY_MS) : null;
